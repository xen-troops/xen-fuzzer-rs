...
pub fn my_output(s: &str) {
    // We ignore result, because there actually little we can do in
    // this case. We can get fault here becase QEMU configures
    let _res = writeln!(io::stdout(), "{s}");
}
...
pub fn fuzz() {
    ...
    let monitor = MultiMonitor::new(my_output);
