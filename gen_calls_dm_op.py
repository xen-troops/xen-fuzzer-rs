...
KNOWN_HTPES = {"XEN_DMOP": ("dmop", "__HYPERVISOR_dm_op")}
...
@dataclass
class Dm_opMainStruct:
    node: Cursor
    ops: List[Dm_opOpStruct]
...
def handle_macro(node: Cursor):
    ...
    if node.extent.start in MAIN_STRUCT.extent:
        DM_OP_OP_IDS.append(node.spelling)
...
def handle_struct(node):
    ...
    #     return
    global MAIN_STRUCT
    if node.spelling == "xen_dm_op" and PASS == 0:
        print("Found main struct")
        #        pprint(get_info(node,1))
        MAIN_STRUCT = node
        return

    LEARNED_STRUCTS.append(Dm_opOpStruct(node=node))


def find_struct(name: str) -> Dm_opOpStruct:
    for x in LEARNED_STRUCTS:
        print(x.node.spelling)
                ...
                for u in ch.get_children().__next__().get_children():
                    ...
                    if struct:
                        if u.spelling in op_replacement.keys():
                            ...
                            ops.append(("XEN_DMOP_" + u.spelling, u.spelling,
                                        struct))
    return Dm_opMainStruct(node=node, ops=ops)
...
def handle_main_union(u):
    ...
    op_struct_name = fdec[0].spelling.removeprefix("struct ").removesuffix("_t")
    s = find_struct(op_struct_name)
    if s:
        return s
    else:
        raise Exception(f"Can't find struct {op_struct_name} for op {u.spelling}")
...
def emit_hypercall_def(op: str, sname: str, d: Dm_opOpStruct):
    skips = [
    ]
    ...
    out.write(f"hypercall! {{{op.lower()}, __HYPERVISOR_dm_op,\n")
    out.write("        hypercall_arg!{0, complex_struct xen_dm_op,\n")
    out.write(
        f"            hypercall_struct_field!{{const xen_dm_op:op (uint32_t) = {op}}},\n"
    )
...
def parse_fields(d: Dm_opOpStruct) -> List[HypercallField]:
...
def emit_hypercall_field(struct_name: str, f: HypercallField):
    ...
    return (
        " " * 12 +
        f"hypercall_struct_field!{{{fkind_str} xen_dm_op:u.{struct_name}.{f.fname}{type_str}{fbufsize_str}}}"
    )
